    ...
    const steps = [
        {
            ...
            description: "Cuéntanos tus necesidades de salud, movilidad y preferencias sociales en 2 minutos.",
            icon: <ClipboardList className="h-8 w-8 text-white" />,
        },
        {
            number: 2,
            title: "Diseña tu estancia ideal",
            description: "Tu gestor personal te contactará para ajustar cada detalle de tu aterrizaje en España.",
            icon: <MessageCircle className="h-8 w-8 text-white" />,
        },
        {
            number: 3,
            title: "Nosotros nos encargamos de todo",
            description: "Logística, sanidad y comunidad listas para tu llegada. Tú solo disfruta del sol.",
            ...
            <div className="container mx-auto px-4">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center max-w-7xl mx-auto">
                    ...
                    <ol className="space-y-6">
                        {steps.map((step) => (
                            <li key={step.number} className="group">
                                <Card
                                    className="relative border-none shadow-sm hover:shadow-md transition-all duration-300 bg-slate-50 overflow-hidden flex flex-col p-6 h-full border border-slate-100"
                                >
                                    {/* Number Badge (Top Right) */}
                                    <div className="absolute top-6 right-6 bg-[#0D3B3F] text-white w-12 h-12 rounded-full flex items-center justify-center font-bold text-2xl shadow-md z-10 group-hover:scale-110 transition-transform" aria-hidden="true">
                                        {step.number}
                                    </div>
                    ...
                    <div className="relative h-[600px] w-full rounded-2xl overflow-hidden shadow-2xl">
                        {/* Using standard img tag to bypass Next.js Image issues temporarily */}
                        <img
